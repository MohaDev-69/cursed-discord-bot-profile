                ...
                async interaction =>
                                    {
                                        const 
                                            Dataa
                                                    = await Coins.findOne({userID: interaction.user.id}); 
                                        
                                        const 
                                                RepData 
                                                        = await Rep.findOne({userID: interaction.user.id});
                                        const 
                                                NoteData 
                                         = await Note.findOne({userID: interaction.user.id});
           ...
           const 
                background 
                            = 
                                 await Canvas
                                                .loadImage
                                                            ("https://f.top4top.io/p_21317j91r1.png");
            ctx
                .drawImage
                        (background, 0, 0, 400, 400);
let 
    url
         = 
            interaction
                ...
                ava.
                    getBuffer(jimp.MIME_PNG, (err, buf) => {
                        ...
                        ctx
                            .fillText
                                        (`${interaction.user.username}`, 195, 230)
                        ...
                        ctx
                                                .fillText
                                                            (`${millify(CoinsData, {precision: 2, decimalSeparator: ","})}`, 50,255)
                    ...
                    = "#16bbc7";
                        ...
                        ctx
                                            .fillText
                    (`${millify(RepDataa, {precision: 2, decimalSeparator: ","})}`, 360,260)
...
(interaction
                    ...
                    .bot 
                     ||
                      MentiondUser
                       == 
                       interaction.user)
                       return interaction.editReply(`❌ - this user is bot or this user trying to give him-self rep`)
    let MentiondUserData 
                                        = 
                                        await
                                         Rep
                                         .findOne({ userID: MentiondUser.id });
    let ThisUser 
    =
     RepData;
    if(!ThisUser){
    ...
    if(!MentiondUserData){
        await
         new
          Rep({userID : MentiondUser.id , rep : 0, cooldown : "1"}).save();
    }
